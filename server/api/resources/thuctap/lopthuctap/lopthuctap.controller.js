...
import * as Service from './lopthuctap.service';
import Model from './lopthuctap.model';
...
export async function update(req, res) {
  try {
    ...
    const data = await Model.findOneAndUpdate({ _id: id }, value, { new: true })
      .populate({path: 'dotthuctap_id'})
      .populate({path: 'lophoc_id', select:'ten_lop_hoc'});
    if (!data) {
...
export async function create(req, res) {
  ...
  try {
    ...
    let dataRtn = await data
      .populate({ path: 'dotthuctap_id' })
      .populate( { path: 'lophoc_id', select: 'ten_lop_hoc' }).execPopulate();
...
export async function getAll(req, res) {
  try {
    ...
    options.populate = [
      { path: 'dotthuctap_id' },
      { path: 'lophoc_id', select: 'ten_lop_hoc' },
    ];
